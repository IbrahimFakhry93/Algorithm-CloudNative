...
{
    int size = freq.size();
    int freqArray[100][2]; // assuming max 100 unique characters; adjust as needed
    int i = 0;
    ...
    sort(freqArray, 0, size - 1);

    cout << "Print Sorted data ..." << endl;
    for (int j = 0; j < size; j++)
    {
        cout << (char)freqArray[j][0] << " " << freqArray[j][1] << endl;
    }
    // No need to free memory for stack array
}
...
void CharFreq::merge(int array[][2], int start, int mid, int end)
{
    ...
    {
        left_array[i] = new int[2];
    }
    int **right_array = new int *[right_length];
    for (j = 0; j < right_length; j++)
    {
        right_array[j] = new int[2];
    }
