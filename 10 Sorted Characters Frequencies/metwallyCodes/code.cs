...
public class CharFreq
{
    ...
    public void ASCIIMethod(string message)
    {
        ...
        {
            int current_code = (int)message[i]; //* convert the current character to ASCII code
            freq[current_code]++;
        }
    ...
    public void AnyCodeMethod(string message)
    {
        ...
        for (int i = 0; i < message.Length; i++)
        {
            ...
            {
                freq[message[i]] = (int)freq[message[i]] + 1;  //* the value freq[message[i]] in hash-table is object so we have to convert it to integer to increment it
            }
        }
